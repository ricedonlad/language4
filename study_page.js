...
// 전체 학습 모드에서 한 바퀴 완료되었는지 추적하는 플래그 (중단점 저장 방지용)
let isFullStudyCompletedOnce = false; // 한 번이라도 모든 단어를 완주했음을 기록 (local storage와 연동)
...
function clearDayFavorites() {
    ...
    let confirmMessage;
    let initialFavoriteCount = words.filter(word => word.isFavorite).length;
    ...
    } else { // 전체 학습 (Day가 null인 일반 모드 또는 즐겨찾기 모드)
...
function goHome() {
    ...
    if (currentStudyDay !== null) {
        ...
        // Day 학습에서 홈으로 이동 시 전체 학습 및 즐겨찾기 학습 중단점 초기화
    ...
    // currentStudyDay가 null인 경우 (전체 학습 또는 즐겨찾기 모드)
        ...
        // 전체 학습 모드 (isGeneralMode === true)
        if (isGeneralMode) {
            // 전체 학습이 한 바퀴 완료되지 않았다면 중단점 저장
            if (!isFullStudyCompletedOnce) { // isFullStudyCompletedOnce는 `nextBtn` 클릭 시 마지막에서만 `true`가 됨
                ...
                // 여기서는 그냥 유지 (main 페이지에서 읽을 수 있도록)
            }
            // 전체 학습에서 홈으로 이동 시 Day 및 즐겨찾기 학습 중단점 초기화
            localStorage.removeItem('lastInterruptedStudyDay');
            localStorage.removeItem('lastInterruptedStudyIndex');
            localStorage.removeItem('lastInterruptedFavoriteStudyIndex');
        } 
        // 즐겨찾기 모드 (isGeneralMode === false)
            ...
            // 즐겨찾기 학습에서 홈으로 이동 시 Day 및 전체 학습 중단점 초기화
            localStorage.removeItem('lastInterruptedStudyDay');
            localStorage.removeItem('lastInterruptedStudyIndex');
            localStorage.removeItem('lastInterruptedFullStudyIndex');
            localStorage.removeItem('isFullStudyCompletedFlag'); // 전체 학습 완료 플래그도 클리어
    ...
    localStorage.removeItem('currentStudyDay'); // study_page.js 초기화 시 currentStudyDay를 다시 로드하므로 여기서 지움
...
swipeInteractionArea.addEventListener('touchstart', (e) => {
    if (e.touches.length > 0) { // 터치 이벤트가 발생했는지 확인
...
swipeInteractionArea.addEventListener('touchend', (e) => {
    ...
    if (isSwiping && Math.abs(deltaX) > swipeThreshold) { 
        // 스와이프 방향 로직: 오른쪽으로 스와이프 시 이전, 왼쪽으로 스와이프 시 다음
        if (deltaX > 0) { // 오른쪽으로 스와이프
            prevBtn.click(); 
            console.log('Swipe detected: Prev word (swiped right)');
        } else { // 왼쪽으로 스와이프
            nextBtn.click(); 
            console.log('Swipe detected: Next word (swiped left)');
        }
    } else {
...
nextBtn.addEventListener('click', () => {
    ...
    if (currentStudyDay !== null) { // Day 학습 모드일 경우 (Day 경계 유지)
        ...
        if (currentIndex < currentWords.length - 1) {
            ...
            // 전체 학습 모드(isGeneralMode === true && currentStudyDay === null)에서만 완료 알림
            if (isGeneralMode && currentStudyDay === null && currentWords.length > 0) {
                alert('축하합니다! 모든 학습 과정을 완료했습니다!');
                localStorage.setItem('isFullStudyCompletedFlag', 'true'); // 한 바퀴 완료 플래그 설정
                goHome(); // 첫 페이지로 이동
                return; // 함수 종료
            }
        }
...
window.addEventListener('load', () => {
    ...
    // 5. 일반적인 모드 (lastAppMode)에 따른 인덱스 로드
    ...
    if (navigateToWord) { // 1. 검색 결과로 이동 (가장 높은 우선순위)
        ...
        initialModeIsGeneral = true; // Day 학습은 항상 일반 모드로 시작
...
window.addEventListener('beforeunload', () => {
    ...
    if (currentStudyDay !== null && !isCurrentDayCompleted) {
        ...
        localStorage.setItem('lastInterruptedStudyIndex', currentIndex);
        console.log(`[UNLOAD] Interrupted Day session saved: Day ${currentStudyDay}, Index ${currentIndex}`);
        // Day 학습 중단 시, 다른 학습 중단점은 클리어 (둘 중 하나만 유지)
        localStorage.removeItem('lastInterruptedFullStudyIndex');
        localStorage.removeItem('lastInterruptedFavoriteStudyIndex');
        localStorage.removeItem('isFullStudyCompletedFlag'); // 전체 학습 완료 플래그도 클리어
    } 
    // 전체 학습 모드 (currentStudyDay === null && isGeneralMode === true)에서 한 바퀴를 다 돌지 않은 경우
    else if (currentStudyDay === null && isGeneralMode && !isFullStudyCompletedOnce) { 
        localStorage.setItem('lastInterruptedFullStudyIndex', currentIndex);
        console.log(`[UNLOAD] Interrupted Full Study session saved: Index ${currentIndex}`);
        // 다른 학습 중단점은 클리어
        localStorage.removeItem('lastInterruptedStudyDay');
        localStorage.removeItem('lastInterruptedStudyIndex');
        localStorage.removeItem('lastInterruptedFavoriteStudyIndex');
        // isFullStudyCompletedFlag는 유지 (main 페이지에서 읽을 수 있도록)
    }
    // 즐겨찾기 학습 모드 (currentStudyDay === null && isGeneralMode === false)에서 중단된 경우
    else if (currentStudyDay === null && !isGeneralMode) { 
        localStorage.setItem('lastInterruptedFavoriteStudyIndex', currentIndex);
        console.log(`[UNLOAD] Interrupted Favorite Study session saved: Index ${currentIndex}`);
        // 다른 학습 중단점은 클리어
        localStorage.removeItem('lastInterruptedStudyDay');
    ...
    else { // 학습 완료 등으로 중단할 필요 없을 때 또는 특정 학습이 완료되었을 때 모두 클리어
        localStorage.removeItem('lastInterruptedStudyDay');
        localStorage.removeItem('lastInterruptedStudyIndex');
        localStorage.removeItem('lastInterruptedFullStudyIndex');
        localStorage.removeItem('lastInterruptedFavoriteStudyIndex');
        localStorage.removeItem('isFullStudyCompletedFlag'); // 이 경우 지워야 함 (이미 완료 알림 후 이동이거나, 다른 상황에서 완전히 초기화되므로)
        console.log('[UNLOAD] No interrupted session to save or session completed (or already handled by completion alert). Interruption data cleared.');
    }
});